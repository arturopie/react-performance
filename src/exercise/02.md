...
I wonder if we don't use Web Workers as much because we usually paginate long list at the backend, so our "Worker" is the backend. Only when the list is static, we get the benefit from using "Worker"?
